...
package multiline_logparser_win
...
func findfile(root string) []string {
	...
	exprOld := filepath.Base(root) //*.log
	var expr string
	...
	if exprOld[:1] == "*" || exprOld[:2] == "**" {
		if exprOld[:1] == "*" {
			...
			if strings.Split(exprOld, "*")[1] != ".log" {
				expr = "." + strings.Replace(exprOld, ".", "\\.", -1)
			}
...
func (l *LogParserPlugin) Start(acc telegraf.Accumulator) error {
	...
	for _, filename := range l.Files {
		...
		// 初始化当前文件缓存记录文件地址
		filewatch.watchfile = filepath.Join(watchdir, filepath.Base(filename)) + ".cache"
		...
		if _, err := os.Stat(filename); os.IsNotExist(err) {
			...
			// 获取当前文件inode
			inode, err := queryFilenameById(filename)
			if err != nil {
				panic(err)
			}
			filewatch.inode = inode
			// 初始化md5
		...
		for file, v := range fileLine {
			...
			if timeSub > time.Duration(l.Timeout)*time.Second {
				if fileLine[file].last != "" {
					...
					m, err := l.GrokParser.ParseLine(fileLine[file].last)

					if err == nil {
						if m != nil {
							tags := m.Tags()
							tags["path"] = file
							instance := strings.Split(file, "/")
							filename := instance[len(instance)-1]
							instance = strings.Split(filename, ".log")
							...
							//fmt.Println(file)
							// 发送完再记录位置，防止程序意外停止造成数据缺失
							tailer := l.tailers[file]
							offset, _ := tailer.Tell()
							inode, err := queryFilenameById(file)
							if err != nil {
								panic(err)
							}
							//fmt.Println(tailer.Filename)
							filewatchMap[file].inode = inode
							filewatchMap[file].watchmd5 = md5Str(file + strconv.FormatUint(inode, 10))
							//重新记录到文件
							f, err1 := os.OpenFile(filewatchMap[file].watchfile, os.O_WRONLY|os.O_TRUNC, 0644)
							if err1 != nil {
								panic(err1)
							}
...
func (l *LogParserPlugin) receiver(tailer *tail.Tail) {
	...
	for line = range tailer.Lines {
		...
		// 每次都重新校验md5，且记录到缓存文件。
		inode, err := queryFilenameById(tailer.Filename)
		if err != nil {
			panic(err)
		}
		//fmt.Println(tailer.Filename)
		filewatchMap[tailer.Filename].inode = inode
	...
	for {
		...
		if err == nil {
			if m != nil {
				...
				//instance := entry.path.Split("/")[len(entry.path.Split("/"))-1]
				instance := strings.Split(entry.path, "/")
				filename := instance[len(instance)-1]
				instance = strings.Split(filename, ".log")
				tags["instance"] = (instance[0])
				m.AddField("msg", entry.line)
				l.acc.AddFields(m.Name(), m.Fields(), tags, m.Time())
				if l.Tail_interval > 0 {
				...
				offset, _ := tailer.Tell()
				inode, err := queryFilenameById(entry.path)
				if err != nil {
					panic(err)
				}
				//fmt.Println(tailer.Filename)
				filewatchMap[entry.path].inode = inode
				filewatchMap[entry.path].watchmd5 = md5Str(entry.path + strconv.FormatUint(inode, 10))
				//重新记录到文件
				f, err1 := os.OpenFile(filewatchMap[entry.path].watchfile, os.O_WRONLY|os.O_TRUNC, 0644)
				if err1 != nil {
					panic(err1)
				}
				content := strconv.FormatUint(inode, 10) + "\n" + strconv.FormatInt(offset, 10) + "\n" + filewatchMap[tailer.Filename].watchmd5
...
func (l *LogParserPlugin) multiline_parser() {
	defer l.wg.Done()
...
func init() {
	inputs.Add("multiline_logparser_win", func() telegraf.Input {
		return NewLogParser()
	})
}
...
func queryFilenameById(fname string) (uint64, error) {
	...
	if err = syscall.GetFileInformationByHandle(syscall.Handle(hFile), &data); err != nil {
		return 0, err
	}
