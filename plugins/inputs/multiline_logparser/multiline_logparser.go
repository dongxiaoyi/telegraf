...
func (l *LogParserPlugin) Start(acc telegraf.Accumulator) error {
	...
	for _, filename := range l.Files {
		...
		// 初始化当前文件缓存记录文件地址
		filewatch.watchfile = path.Join(watchdir, path.Base(filename)) + ".cache"
				...
				if fileLine[file].last != "" {
					...
					if err == nil {
						if m != nil {
							...
							var filestat syscall.Stat_t
							if err := syscall.Stat(file, &filestat); err != nil {
								panic(err)
							}
							//fmt.Println(tailer.Filename)
							inode := filestat.Ino
							filewatchMap[file].inode = filestat.Ino
	...
	for {
		...
		if err == nil {
			if m != nil {
				...
				offset, _ := tailer.Tell()
				var filestat syscall.Stat_t
				if err := syscall.Stat(entry.path, &filestat); err != nil {
					panic(err)
				}
				//fmt.Println(tailer.Filename)
				inode := filestat.Ino
				filewatchMap[entry.path].inode = filestat.Ino
...
func init() {
	inputs.Add("multiline_logparser", func() telegraf.Input {
		return NewLogParser()
	})
}
